...
void plotar_densidade_halo() {
    
    TGraph *g = new TGraph("dados_densidade_halo.txt");

    //Título
    g->SetTitle("Perfil de Densidade Vertical da Galaxia;z [kpc];n(z) / n_{0}");
    g->SetLineWidth(1);
    ...
    g->SetLineColor(kAzure+1); // Cor do gráfico 

    c1->SetGrid();
    g->Draw("AC");
    
    //Salva o gráfico como imagem na pasta
    c1->SaveAs("perfil_densidade_halo.png");
}
